...
if(!exists("run30")) 
  run30 = runAnalysis(
    ...
    bootstraps = 100,ageTolerance = 3,specimenDateTolerance=5,
    modelFormula = models,
    includeRaw=TRUE,includeMatched = TRUE)


p1 = ggplot(run30$rawMatchedData, aes(x=time,colour=sGene,linetype=deathStatus))+geom_line(stat="count")+scale_y_log10()
p2 = ggplot(run30$rawMatchedData %>% filter(status==0),aes(x=time,colour=sGene))+ stat_ecdf(geom = "step")+ylab("P(censored before t|censored)")
p3 = ggplot(run30$rawMatchedData %>% filter(status==1),aes(x=time,colour=sGene))+ stat_ecdf(geom = "step")+ylab("P(died before t|died)")
...
run30$table5Data %>% group_by(model,term) %>% 
    summarise(
      across(
