...
* May want to reduce number of bootstraps for testing
...
### Estimate HR using weighted paired matches approach

```{r}
# https://ete-online.biomedcentral.com/articles/10.1186/s12982-017-0060-8
# Weighted using a modification of the algorithm above to cope with duplicates in different numbers of matched pairs.
PLME_HR = runAnalysisQuick(
  coxData, sGeneCtThreshold = 30, ctThreshold = 30,
  ageTolerance = 3,specimenDateTolerance=5
)

PLME_HR %>% prettyPrintSummary()

# Unweighted on bootstrapped paired
UnwtPLME_HR = run30$table5Data %>% group_by(model,term) %>% 
    summarise(
      across(
...
UnwtPLME_HR %>% prettyPrintSummary()
```
...
```{r}
tmp = run30$table4Data %>% filter(model=="S gene only") %>% group_by(boot) %>% summarise(mean.HR = mean(HR)) %>% ungroup() 
...
tmp = bootstrappedCoxModel(run30$rawMatchedData, "Full model", Surv(time,status) ~ sGene+age+sex+imd_decile+ethnicity_final+CT_N)
...
run30$pairedMatchedUnfilteredData %>% group_by(FINALID.neg) %>% summarise(count = n_distinct(FINALID.pos)) %>% ungroup() %>% group_by(count) %>% count()
run30$pairedMatchedUnfilteredData %>% group_by(FINALID.pos) %>% summarise(count = n_distinct(FINALID.neg)) %>% ungroup() %>% group_by(count) %>% count()
...
run30drop = runAnalysis(
    coxData, sGeneCtThreshold = 30, ctThreshold = 30,
    bootstraps = 10,ageTolerance = 3,specimenDateTolerance=5,
    modelFormula = sGeneOnly,resolutionStrategy = "drop")
...
run30none = runAnalysis(
    coxData, sGeneCtThreshold = 30, ctThreshold = 30,
    bootstraps = 10,ageTolerance = 3,specimenDateTolerance=5,
    modelFormula = sGeneOnly,resolutionStrategy = "none")
...
run30drop2 = runAnalysis(
    coxData, sGeneCtThreshold = 30, ctThreshold = 30,
    bootstraps = 10,ageTolerance = 3,specimenDateTolerance=5,
    modelFormula = sGeneOnly,resolutionStrategy = "drop",ratio=2)
...
run30drop3 = runAnalysis(
    coxData, sGeneCtThreshold = 30, ctThreshold = 30,
    bootstraps = 10,ageTolerance = 3,specimenDateTolerance=5,
    modelFormula = sGeneOnly,resolutionStrategy = "drop",ratio.neg=1,ratio.pos=2)
